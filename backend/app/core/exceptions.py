...
class AppException(Exception):
    def __init__(self, message: str, status_code: int = status.HTTP_400_BAD_REQUEST, errors: object | None = None):
...
def error_response(message: str, errors: object | None = None) -> dict[str, object]:
...
def register_exception_handlers(app: FastAPI) -> None:
    ...
    async def integrity_exception_handler(_: Request, __: IntegrityError) -> JSONResponse:
        return JSONResponse(
            ...
            content=error_response("A database constraint was violated"),
        )
